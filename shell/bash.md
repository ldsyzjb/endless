...
CONDITIONAL EXPRESSIONS
       ...
       or binary primaries.  Bash handles several filenames specially when they
       are  used in expressions.  If the operating system on which bash is run‐
       ning provides these special files, bash will use them; otherwise it will
       emulate  them internally with this behavior: If any file argument to one
...
COMMAND EXECUTION
       ...
       mand_not_found_handle.   If that function exists, it is invoked with the
...
JOB CONTROL
       ...
       line.  For example, %ce refers to a stopped ce job.  If a prefix matches
       more  than  one  job,  bash  reports an error.  Using %?ce, on the other
       hand, refers to any job containing the string ce in  its  command  line.
       If  the substring matches more than one job, bash reports an error.  The
       ...
       background.  The previous job may be referenced using %-.  If  there  is
       only  a single job, %+ and %- can both be used to refer to that job.  In
...
PROMPTING
       ...
       strings to be customized by inserting a number of backslash-escaped spe‐
...
READLINE
   ...
   Readline Initialization
       ...
       tialization  file.   Blank  lines are ignored.  Lines beginning with a #
   ...
   Readline Variables
       ...
       keyseq-timeout (500)
              ...
              until another key is pressed to decide which key sequence to com‐
              plete.
       mark-directories (On)
   ...
   Readline Conditional Constructs
       ...
       $if    The $if construct allows bindings to be made based on the editing
              ...
              The  text  of the test extends to the end of the line; no charac‐
   ...
   Commands for Manipulating the History
       ...
       edit-and-execute-command (C-xC-e)
              ...
              result as shell commands.  Bash attempts to invoke $VISUAL, $EDI‐
   ...
   Miscellaneous
       ...
       glob-expand-word (C-x *)
              ...
              the  word.   If  a  numeric  argument is supplied, an asterisk is
   ...
   Programmable Completion
       ...
       the  completion  code  as  the  full  set  of possible completions.  The
       ...
       cuted),  programmable  completion  restarts  from the beginning, with an
...
HISTORY
       ...
       of lines specified by the value of  HISTFILESIZE.   If  HISTFILESIZE  is
       ...
       preserved across shell sessions.  This uses the history comment  charac‐
       ter  to  distinguish  timestamps from other history lines.  After saving
       the history, the history file is truncated to contain no more than HIST‐
       FILESIZE lines.  If HISTFILESIZE is unset, or set to null, a non-numeric
...
HISTORY EXPANSION
       ...
       expansion character, but the history expansion character is also treated
   ...
   Modifiers
       ...
       s/old/new/
              ...
              will quote the &.  If old is null, it is set to the last old sub‐
              stituted,  or,  if  no previous history substitutions took place,
...
SHELL BUILTIN COMMANDS
       ...
       source filename [arguments]
              ...
              is  not  in  posix  mode, the current directory is searched if no
              file is found in PATH.  If the sourcepath  option  to  the  shopt
       ...
       bg [jobspec ...]
              ...
              enabled, any specified jobspec was not found or was started with‐
              out job control.
       ...
       bind [-m keymap] keyseq:readline-command
              ...
              -x keyseq:shell-command
                     ...
                     line buffer and the READLINE_POINT variable to the current
       ...
       complete -pr [-DE] [name ...]
              ...
              tion specifications.  The -D option indicates that the  remaining
       ...
       compopt [-o option] [-DE] [+o option] [name]
              ...
              The -D option indicates that the remaining options  should  apply
       ...
       typeset [-aAfFgilnrtux] [-p] [name[=value] ...]
              ...
              able and +r will not remove the readonly attribute.  When used in
              a function, declare and typeset make each name local, as with the
              local  command,  unless the -g option is supplied.  If a variable
              name is followed by =value, the value of the variable is  set  to
              value.  When using -a or -A and the compound assignment syntax to
              create array variables, additional attributes do not take  effect
              until  subsequent  assignments.   The return value is 0 unless an
              ...
              variable name, an attempt is made to turn off readonly status for
              a readonly variable, an attempt is made to turn off array  status
       ...
       echo [-neE] [arg ...]
              ...
              preted by default.  The xpg_echo shell  option  may  be  used  to
       ...
       getopts optstring name [args]
              ...
              time  the  shell  or  a  shell script is invoked.  When an option
       ...
       local [option] [name[=value] ... | - ]
              ...
              their  original  values when the function returns.  With no oper‐
       ...
       printf [-v var] format [arguments]
              ...
              tion to the  standard  printf(1)  format  specifications,  printf
       ...
       return [n]
              ...
              RETURN trap is executed before execution resumes after the  func‐
              tion or script.
       ...
       shopt [-pqsu] [-o] [optname ...]
              ...
              compat31
                      ...
                      command's =~ operator and locale-specific string compari‐
                      son when using the [[ conditional command's < and > oper‐
                      ators.  Bash versions prior to bash-4.1 use ASCII  colla‐
              ...
              compat41
                      ...
                      in a double-quoted parameter expansion as a special char‐
                      acter.  The single quotes must match (an even number) and
                      the  characters  between the single quotes are considered
                      quoted.  This is the behavior of posix mode through  ver‐
                      sion 4.1.  The default bash behavior remains as in previ‐
                      ous versions.
              compat42
              ...
              expand_aliases
                      ...
                      tive shells.
              extdebug
                      If set at shell invocation, arrange to execute the debug‐
                      ger profile before the shell  starts,  identical  to  the
       ...
       trap [-lp] [[arg] sigspec ...]
              ...
              single sigspec) or -, each specified signal is reset to its orig‐
              inal  disposition  (the value it had upon entrance to the shell).
              ...
              mand  executes  in  a  shell  function (see SHELL GRAMMAR above).
